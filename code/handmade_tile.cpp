 ...
 *
 *Author:  Rob Graham.
 *Created: Sun Nov 19 19:05:47 2017
             ...
             memory_arena *Arena = 0)
{
    ...
    // TODO(casey): Write a better hash function.
    ...
    do
    {
        ...
        if(Arena && (Chunk->TileChunkX != TILE_CHUNK_UNITIALIZED) && (!Chunk->NextInHash))
        {
            Chunk->NextInHash = PushStruct(Arena, tile_chunk);
            Chunk = Chunk->NextInHash;
            Chunk->TileChunkX = TILE_CHUNK_UNITIALIZED;
        }
        
        if(Arena && (Chunk->TileChunkX == TILE_CHUNK_UNITIALIZED))
        {
            ...
            // TODO(Rob): Do we want to always initialize?
...
//
// TODO(Rob): Do these really belong in more of a "positioning" or "geometry" file?
//
...
RecanonicalizeCoord(tile_map *TileMap, int32 *Tile, real32 *TileRel)
{
    ...
    // NOTE(Rob): TileMap is assumed to be toroidal topology, if you
...
InitializeTileMap(tile_map *TileMap, real32 TileSideInMeters)
{
    ...
    for(uint32 TileChunkIndex = 0;
        ...
        ++TileChunkIndex)
    {
        TileMap->TileChunkHash[TileChunkIndex].TileChunkX = TILE_CHUNK_UNITIALIZED;
    }
}